...
<body class="relative min-h-screen pb-[200px] bg-fixed bg-no-repeat bg-center bg-cover bg-[#E7F4E4]">
	...
	<div class="container mx-auto p-6 mt-12">
		...
		<div class="mb-4">
	        ...
	        <a href="insert.php?type=golongan&page=<?php echo $page; ?>" class="bg-green-500 text-white px-6 py-2 rounded-lg hover:bg-green-600 transition duration-300">Tambah Golongan</a>
	    </div>
				...
				<?php foreach ($result as $row): ?>
					<tbody>
						<tr class="border-b">
							...
							<td class="py-2 px-4"><?php echo $row['perlu_resep'] ?></td>
							<td class="py-2 px-4 flex justify-center"><img width="200px" height="200px" src="<?php echo $row['gambar'];?>" alt=""></td>
							<td class="py-2 px-4">
								<!-- Tombol Edit -->
								<a href="edit.php?type=golongan&id=<?php echo $row['id'] ?>&page=<?php echo $page;?>" class="text-blue-500 hover:text-blue-700 font-semibold">Edit</a> 
								|
								<!-- Tombol Hapus -->
    ...
    <script>
        function showDeleteAlert(id, page) {
            ...
            document.getElementById('deleteLink').setAttribute('href', 'delete.php?type=golongan&id='+id+'&page='+page);
        }
