        ...
        <form action="" method="POST" enctype="multipart/form-data">
        	<?php foreach ($result as $row): ?>
        		<div class="mb-4">
	                ...
	                <input type="text" id="nama" name="nama" class="w-full px-4 py-2 border rounded-lg" value="<?php echo $row['nama'] ?>" required>
	            ...
	            <div class="mb-4">
	                ...
	                <select id="kategori" name="kategori" class="w-full px-4 py-2 border rounded-lg" required>
	                    ...
	                    <?php foreach ($result_kategori as $row_kategori): ?>
	                        <option value="<?php echo $row_kategori['id']; ?>" <?php echo ($row_kategori['id'] == $row['kategori_id']) ? 'selected' : '' ; ?>><?php echo $row_kategori['kategori']; ?></option>
	                    <?php endforeach ?>
	                </select>
	            </div>
	            <div class="mb-4">
	                ...
	                <select id="golongan" name="golongan" class="w-full px-4 py-2 border rounded-lg" required>
	                    ...
	                    <?php foreach ($result_golongan as $row_golongan): ?>
	                        <option value="<?php echo $row_golongan['id']; ?>" <?php echo ($row_golongan['id'] == $row['golongan_id']) ? 'selected' : '' ; ?>><?php echo $row_golongan['golongan']; ?></option>
	                    <?php endforeach ?>
	                </select>
	            </div>
	            <div class="mb-4">
	                <label for="produsen" class="block text-sm font-medium">Produsen</label>
	                <input type="text" id="produsen" name="produsen" class="w-full px-4 py-2 border rounded-lg" required value="<?php echo $row['produsen'] ?>">
	            </div>
	            <div class="mb-4">
	                <label for="harga" class="block text-sm font-medium">Harga</label>
	                <input type="number" id="harga" name="harga" class="w-full px-4 py-2 border rounded-lg" required value="<?php echo $row['harga'] ?>">
	            </div>
	            <div class="mb-4">
	                <label for="stok" class="block text-sm font-medium">Stok</label>
	                <input type="number" id="stok" name="stok" class="w-full px-4 py-2 border rounded-lg" required value="<?php echo $row['stok'] ?>">
	            </div>
	            <div class="mb-4">
	                <label for="khasiat" class="block text-sm font-medium">Khasiat</label>
	                <textarea id="khasiat" name="khasiat" rows="4" class="w-full px-4 pt-2 pb-20 border rounded-lg" required><?php echo $row['khasiat'] ?></textarea>
	            </div>
	            <div class="mb-4">
	                <label for="gambar" class="block text-sm font-medium">Gambar Obat (Kosongkan Jika Tidak Memilh)</label>
        	...
        	<?php endforeach ?>
            <button type="submit" class="bg-green-500 text-white w-full px-6 py-2 rounded-lg hover:bg-green-600 transition duration-300 mb-4">Update Obat</button>
